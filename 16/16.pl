...
% !!!
% now adding some features in order to solve the power digit sum problem
% !!!

power_product(List, _, 0, List).
power_product(List, Number, Power, Result):- NewPower is Power - 1, product(List, Number, NewList), power_product(NewList, Number, NewPower, Result).

sum_of_digits([], Sum, Sum).
sum_of_digits([H|T], Sum, Result):- NewSum is Sum + H, sum_of_digits(T, NewSum, Result).

power_digit_sum(Number, Power, Result):- power_product([1], Number, Power, R), sum_of_digits(R, 0, Result).